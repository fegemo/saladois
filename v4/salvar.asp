...
Case "bomburgoes"
  Select Case Request.QueryString("cod")
	Case "-0"
	  'Nova Categoria
	  Rs.Source = "SELECT * FROM TblCategorias"
	  Rs.Open()
	  Rs.AddNew
	  Rs("NomeCategoria") = Request.Form("Nome")
	  Rs("SistemaCategoria") = 0
	  If IsNumeric(Request.Form("Sistema")) Then
	  	Rs("SistemaCategoria") = Request.Form("Sistema")
	  End If
	  Rs.Update
	  Rs.Close()
	Case "0"
	  'Novo Bomburgão
	  'Verificar se o integrante e a categoria existem
	  If Not vIntegrante(Request.Form("Integrante")) Then
		Response.Redirect "gerenc.asp?func=erro&cod=" & "Integrante não encontrado no banco de dados. Adicione-o e tente novamente"
	  End If
	  If Not vCategoria(Request.Form("Categoria")) Then
		Response.Redirect "gerenc.asp?func=erro&cod=" & "Categoria não encontrada no banco de dados. Adicione-a e tente novamente"
	  End If
	  Rs.Source = "SELECT * FROM TblBomburgoes"
	  Rs.Open()
	  Rs.AddNew
	  Rs("IntegranteBomburgao") = Request.Form("Integrante")
	  Rs("CategoriaBomburgao") = Request.Form("Categoria")
	  Rs("Parametro1Bomburgao") = Request.Form("Parametro1")
	  Rs("Parametro2Bomburgao") = 0
	  If IsNumeric(Request.Form("Parametro2")) Then
	  	Rs("Parametro2Bomburgao") = Request.Form("Parametro2")
	  End If
	  Rs.Update
	  Rs.Close()	  
	Case Else
	  If Mid(Request.QueryString("cod"),1,1) = "-" Then
		'Editar Categoria
	  ...
	  Else
		'Editar Bomburgão
		  ...
		  Response.Redirect "gerenc.asp?func=erro&cod=" & "Categoria não encontrada no banco de dados. Adicione-a e tente novamente"
	  	End If
	  	Rs.Source = "SELECT * FROM TblBomburgoes WHERE CodigoBomburgao=" & Request.QueryString("cod")
	  	Rs.Open()
		If Rs.EOF Then
		  Rs.AddNew
		End If
	  	...
	  	Rs.Update
	  End If
	  If Request.Form("delcategoria") = 1 Then
		Rs.Delete
	  End If
	  If Request.Form("delbomburgao") = 1 Then
		Rs.Delete
	  End If
	  Rs.Close()
  End Select

  'Funções de validação de integrante e de categoria
  Function vIntegrante(CodigoIntegrante)
	lConsulta.Source = "SELECT * FROM TblIntegrantes WHERE CodigoIntegrante=" & CodigoIntegrante
	lConsulta.Open()
	...
	lConsulta.Close()
  End Function
  Function vCategoria(CodigoCategoria)
	lConsulta.Source = "SELECT * FROM TblCategorias WHERE CodigoCategoria=" & CodigoCategoria
	lConsulta.Open()
	If lConsulta.EOF Then
	  vCategoria = False
	Else
	...
	lConsulta.Close()
  End Function
  Response.Redirect "gerenc.asp?func=bomburgoes"
Case "dicionarios"
  If Not Request.QueryString("cod") = "0" Then
	...
	Else
	  ...
	  If Not lConsulta.EOF Then
		...
		Do Until lConsulta.EOF
		  ...
		  'Verificar se é para zerar a enquete
		  If Request.Form("zerenquete") = 1 Then
			lConsulta("VotosOpcao") = 0
		  End If
		  lConsulta.Update
		  lConsulta.MoveNext
		Loop
...
Case "salopiadinhas"
  If Not Request.QueryString("cod") = "0" Then
	'Se a salopiadinha já existir, editá-la...
	Rs.Source = "SELECT * FROM TblSalopiadinhas WHERE CodigoSalopiadinha=" & Request.QueryString("cod")
	Rs.Open()
	If Not Rs.EOF Then
	  'Editar...
	  Rs("TextoSalopiadinha") = Request.Form("Texto")
	  Rs("AutorSalopiadinha") = Request.Form("Autor")
	  Rs.Update
	Else
	  'Se a salopiadinha não existir, criá-la
	  Rs.AddNew()
	  Rs("TextoSalopiadinha") = Request.Form("Texto")
	  Rs("AutorSalopiadinha") = Request.Form("Autor")
	  Rs.Update
	End If
	'Verificar se é para ser deletado
	If Request.Form("delsalopiadinha") = 1 Then
	  Rs.Delete
	End If
	Rs.Close()
  Else
	'Se for uma nova salopiadinha
	...
	Rs.Close
  End If
  Response.Redirect "gerenc.asp?func=salopiadinhas"
Case "usuarios"
